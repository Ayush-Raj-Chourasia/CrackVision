...
- A Vercel-ready frontend in `frontend/`.
...
2. Deploy the frontend to Vercel from `frontend/`.
...
Set `CRACKVISION_API_URL` in `frontend/.env.local` before starting the frontend.
...
- If you want a polished presentation, drop the Google Stitch UI into `frontend/` and connect it to the API route.
